...
function looksLikeDsl(step: string): boolean {
  return /^(?:@\w|\[(?:SETUP|END SETUP|TEARDOWN|END TEARDOWN)\]|STEP\s+\d*\s*:|DONE\.|NAVIGATE\b|OPEN APP\b|WAIT\b|VERIFY\b|DEBUG\b|DEBUG VARS\b|EXTRACT\b|CALL PYTHON\b|SET\b|USE\b|Click\b|DOUBLE CLICK\b|Fill\b|Type\b|Select\b|Check\b|Uncheck\b|HOVER\b|Drag\b|RIGHT CLICK\b|PRESS\b|MOCK\b|SCAN PAGE\b|IF\b|ELIF\b|ELSE\b|REPEAT\b|FOR EACH\b|WHILE\b)/u.test(
    step,
  );
}
