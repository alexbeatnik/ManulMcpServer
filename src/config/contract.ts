...
export const dslContract: ManulDslContract = {
  version: '0.0.9.33',
  generatedFrom:
    'ManulEngine 0.0.9.33 — manul_engine/helpers.py :: classify_step(), detect_mode(), parse_contextual_hint(); manul_engine/core.py :: run_mission(); manul_engine/cli.py :: parse_hunt_file(); manul_engine/actions.py :: _ActionsMixin; manul_engine/scoring.py :: DOMScorer contextual proximity rules; manul_engine/js_scripts.py :: SNAPSHOT_JS geometry export; manul_engine/imports.py :: parse_import_directive(), resolve_imports(), expand_use_directives(). ManulHeart v0.0.10 — cmd/manul/agent_cmds.go :: engineSchema() (CALL GO, PRINT, SCREENSHOT, WAIT_FOR, explicit END IF/REPEAT/FOR/WHILE terminators, CALL <block>); docs/dsl-for-llms.md.',
  commands: [
    {
      id: 'navigate',
      label: 'NAVIGATE',
    ...
    },
    {
      ...
      description: 'ManulEngine (Python runtime) only. Executes a synchronous Python function inline. Supports positional arguments (including optional \"with args:\" sugar), optional \"into {var}\" / \"to {var}\" capture, and parser-level @script alias rewriting for CALL PYTHON {alias}.func syntax. Module resolution order: hunt dir → CWD → sys.path.',
      category: 'python',
      engine: 'python',
    },
    {
      id: 'call_go',
      label: 'CALL GO',
      uiText: 'CALL GO module.Function',
      snippet: 'CALL GO ${1:module}.${2:Function}${3: ${4:args}}${5: into {${6:variable}}}',
      regex: String.raw`\bCALL\s+GO\b`,
      description: 'ManulHeart (Go runtime) only. Invokes a registered Go handler inline (the Go analogue of CALL PYTHON). Supports positional arguments, optional \"into {var}\" / \"to {var}\" capture, and @script alias rewriting for CALL GO {alias} syntax. Handlers are registered via the pkg/agent RegisterCallHandler extension API.',
      category: 'python',
      engine: 'go',
    },
    {
      id: 'print',
      label: 'PRINT',
      uiText: 'PRINT "message"',
      snippet: 'PRINT "${1:message}"',
      regex: String.raw`^\s*(?:\d+\.\s*)?PRINT\b`,
      description: 'Prints a literal string or a {variable} value to the console/report. Valid as a top-level action line and inside [SETUP]/[TEARDOWN] hook blocks.',
      category: 'utility',
      engine: 'both',
    },
    {
      ...
      uiText: 'SCREENSHOT',
      snippet: 'SCREENSHOT',
      regex: String.raw`^\s*(?:\d+\.\s*)?SCREENSHOT\b`,
      description: 'ManulHeart (Go runtime) only. Captures a screenshot of the current page at this point in the mission and attaches it to the report.',
      category: 'utility',
      engine: 'go',
    },
    {
      id: 'wait_for_label',
      label: 'WAIT_FOR',
      uiText: "WAIT_FOR 'element'",
      snippet: "WAIT_FOR '${1:target}'",
      regex: String.raw`^\s*(?:\d+\.\s*)?WAIT_FOR\s+(?P<quote>["']).+?(?P=quote)\s*$`,
      description: 'ManulHeart (Go runtime) only. Waits until a quoted element becomes present/visible. The Go dialect uses the WAIT_FOR keyword; ManulEngine spells the same intent as "Wait for \'<target>\' to be visible".',
      category: 'wait',
      engine: 'go',
    },
    {
    ...
    },
    {
      ...
      label: 'CALL',
      uiText: 'CALL Login',
      snippet: 'CALL ${1:BlockName}',
      regex: String.raw`^\s*(?:\d+\.\s*)?CALL\s+(?!PYTHON\b|GO\b)[A-Za-z_][\w-]*\s*$`,
      description: 'ManulHeart (Go runtime) only. Invokes a reusable named STEP block / blueprint inline (the Go analogue of USE). Disambiguated from CALL PYTHON / CALL GO by the absence of those keywords.',
      category: 'structure',
      engine: 'go',
    },
    {
    ...
    },
    {
      ...
      uiText: 'END IF',
      snippet: 'END IF',
      regex: String.raw`^\s*(?:\d+\.\s*)?END\s+IF\s*$`,
      description: 'ManulHeart (Go runtime) only. Explicit terminator for an IF/ELIF/ELSE block. ManulEngine closes IF blocks by indentation and does not use END markers.',
      category: 'control_flow',
      engine: 'go',
    },
    {
      ...
      uiText: 'END REPEAT',
      snippet: 'END REPEAT',
      regex: String.raw`^\s*(?:\d+\.\s*)?END\s+REPEAT\s*$`,
      description: 'ManulHeart (Go runtime) only. Explicit terminator for a REPEAT N TIMES loop. ManulEngine closes loops by indentation and does not use END markers.',
      category: 'control_flow',
      engine: 'go',
    },
    {
      ...
      uiText: 'END FOR',
      snippet: 'END FOR',
      regex: String.raw`^\s*(?:\d+\.\s*)?END\s+FOR\s*$`,
      description: 'ManulHeart (Go runtime) only. Explicit terminator for a FOR EACH loop. ManulEngine closes loops by indentation and does not use END markers.',
      category: 'control_flow',
      engine: 'go',
    },
    {
      ...
      uiText: 'END WHILE',
      snippet: 'END WHILE',
      regex: String.raw`^\s*(?:\d+\.\s*)?END\s+WHILE\s*$`,
      description: 'ManulHeart (Go runtime) only. Explicit terminator for a WHILE loop. ManulEngine closes loops by indentation and does not use END markers.',
      category: 'control_flow',
      engine: 'go',
    },
  ],
  ...
  metadata: [
    ...
    },
    {
      ...
      description: 'Declares a file-local helper alias for later CALL usage. On ManulEngine it aliases a Python import path for CALL PYTHON (module alias `@script: {auth} = scripts.auth_helpers` -> `CALL PYTHON {auth}.issue_token`, or callable alias `@script: {issue_token} = scripts.auth_helpers.issue_token` -> `CALL PYTHON {issue_token}`). On ManulHeart it aliases a dotted Go handler path for CALL GO. Alias names must match placeholder identifiers (`^[A-Za-z_]\\w*$`). Targets are dotted import paths only: no `/`, no `\\`, and no `.py` suffix.',
    },
    {
      id: 'data',
      label: '@data:',
